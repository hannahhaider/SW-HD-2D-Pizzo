...
# REMINDER:ask opal how get_mas_path function works
...
k1 = vr[:, 55, 0, 0] #for all phi, at equatorial slice, and initial Ro
k2 = rho[:, 55, 0, 0]
k3 = Pr[:, 55, 0, 0]
k4 = vp[:, 55, 0, 0]
...
for i in range(len(new_r) - 1):
    ...
    if i %25 == 0: #modulo operator, if the division between the incremented r and 25 is equal to 0, then
       ...
       #pcolormesh plots
        #colors = ax[0].pcolormesh(phi_deg, theta_deg, U_SOL[0,:,i+1])
        #fig.colorbar(colors, ax = ax, label = r"$v_{r}$" )
        #colors = plt.imshow(U_SOL[0,:,i+1], extent = [0, 360, 0, 1000], cmap = "plasma", aspect = "auto", origin = "lower" )
        #fig.colorbar(colors)
